...
function openSaleChanceModifyDialog(){
    ...
    if(rows.length==0){
        $.messager.alert("来自crm","请选择待修改的机会数据记录!","warning");
        return;
    }
    if(rows.length>1){
        $.messager.alert("来自crm","暂不支持多条修改记录!","error");
        return;
    }
    $("#fm").form("load",rows[0]);
...
function deleteSaleChance(){
    ...
    //console.log(rows);
    if(rows.length==0){
        $.messager.alert("来自crm","请选择待删除的机会数据记录!","warning");
        return;
    }

    $.messager.confirm("来自crm","您确定删除选中的记录?",function (r) {
        if(r){
            var ids="ids=";
